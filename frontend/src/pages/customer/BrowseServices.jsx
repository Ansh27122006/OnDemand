            ...
            {discount > 0 ? (
              <div className="flex flex-col">
                ...
                <span className="text-lg font-black text-green-600">
                  ...
                  {service.duration && <span className="text-xs text-slate-400 ml-1 font-normal">/ session</span>}
                </span>
              </div>
            ) : (
              <div>
...
const BrowseServices = () => {
  ...
  const filteredServices = useMemo(() => {
    return services.filter((s) => {
      ...
      const matchesVendor = selectedVendor ? s.vendorId?._id === selectedVendor || s.vendorId === selectedVendor : true;
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="bg-white border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">Browse Services</h1>
          <p className="text-slate-500 mt-1 text-sm">
            {loading ? "Loading..." : `${filteredServices.length} service${filteredServices.length !== 1 ? "s" : ""} available`}
          </p>
        </div>
      </div>
