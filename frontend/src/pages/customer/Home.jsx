...
const CategoryBadges = ({ categories, selected, onSelect, accent }) => (
...
const ProductCard = ({ product }) => (
  <div className="flex-shrink-0 w-44 bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100 hover:shadow-md hover:-translate-y-1 transition-all duration-200">
    ...
    <div className="p-3">
      ...
      <Link
        ...
        className="mt-2 block text-center text-xs font-semibold bg-orange-50 text-orange-600 hover:bg-orange-500 hover:text-white rounded-lg py-1.5 transition-colors duration-150">
        View
      </Link>
    </div>
  </div>
...
const ServiceCard = ({ service }) => (
  <div className="flex-shrink-0 w-44 bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100 hover:shadow-md hover:-translate-y-1 transition-all duration-200">
    ...
    <div className="p-3">
      <p className="text-xs font-semibold text-slate-800 truncate">{service.name}</p>
      <p className="text-violet-500 font-bold text-sm mt-0.5">
        ₹{service.price?.toLocaleString("en-IN")}
      </p>
      <Link
        to={`/services/${service._id}`}
        className="mt-2 block text-center text-xs font-semibold bg-violet-50 text-violet-600 hover:bg-violet-500 hover:text-white rounded-lg py-1.5 transition-colors duration-150">
        View
      </Link>
    </div>
  </div>
...
const ScrollRow = ({ items, type, loading }) => {
  if (loading) {
    return (
      <div className="flex gap-4 mt-4">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="flex-shrink-0 w-44 h-48 bg-slate-200 rounded-2xl animate-pulse" />
        ))}
      </div>
    );
  }
  if (!items || items.length === 0) {
    return (
      <div className="mt-4 py-8 text-center bg-white rounded-2xl border border-dashed border-slate-200">
...
const Home = () => {
  ...
  const fetchServicesByCategory = async (category) => {
    ...
    try {
      const { data } = await api.get(`/services/category/${encodeURIComponent(category)}`);
  ...
  const handleProductCatClick = (cat) => {
    setSelectedProductCat(cat);
    fetchProductsByCategory(cat);
  };

  const handleServiceCatClick = (cat) => {
    setSelectedServiceCat(cat);
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        const [ordersRes, bookingsRes, cartRes, prodCatsRes, svcCatsRes] =
          await Promise.all([
            api.get("/orders/my"),
            api.get("/bookings/my"),
            api.get("/cart"),
        ...
        const svcCats = svcCatsRes.data || [];
        setProductCategories(prodCats);
        setServiceCategories(svcCats);

        // auto-load first category on mount
        if (prodCats.length > 0) {
          setSelectedProductCat(prodCats[0]);
          fetchProductsByCategory(prodCats[0]);
        }
        if (svcCats.length > 0) {
          setSelectedServiceCat(svcCats[0]);
          fetchServicesByCategory(svcCats[0]);
        }
      } catch (err) {
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 text-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
          <p className="text-blue-300 text-sm font-semibold mb-1">{greeting} 👋</p>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 pb-12 space-y-10">
        ...
        <section>
          ...
          {statsLoading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <StatCard
                to="/customer/orders" label="Total Orders" value={orders.length}
              ...
              />
              <StatCard
                to="/customer/bookings" label="Total Bookings" value={bookings.length}
              ...
              />
              <StatCard
                to="/cart" label="Items in Cart" value={cartCount}
        ...
        <section>
          <SectionHeader title="Browse Products by Category" to="/products" linkLabel="View all" />
          {productCategories.length === 0 ? (
            <MiniEmpty message="No product categories available yet." />
          ) : (
            <>
              <CategoryBadges
                categories={productCategories}
                selected={selectedProductCat}
                onSelect={handleProductCatClick}
                accent="orange"
              />
              <ScrollRow items={productItems} type="product" loading={productCatLoading} />
            </>
          )}
        </section>
        ...
        <section>
          <SectionHeader title="Browse Services by Category" to="/services" linkLabel="View all" />
          {serviceCategories.length === 0 ? (
            <MiniEmpty message="No service categories available yet." />
          ) : (
            <>
              <CategoryBadges
                categories={serviceCategories}
                selected={selectedServiceCat}
                onSelect={handleServiceCatClick}
                accent="violet"
              />
              <ScrollRow items={serviceItems} type="service" loading={serviceCatLoading} />
            </>
          )}
        </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          {/* Recent Orders */}
          <section>
            <SectionHeader title="Recent Orders" to="/customer/orders" linkLabel="View all" />
            {recentLoading ? (
              ...
              <div className="space-y-3">
                {recentOrders.map((order) => {
                  ...
                  const date = order.createdAt
                    ...
                    : "—";
                  return (
                    <div key={order._id} className="bg-white border border-slate-200 rounded-xl px-4 py-3.5 flex items-center justify-between gap-4 hover:shadow-sm transition-shadow">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="text-xs font-black text-slate-700 font-mono tracking-wider">#{orderId}</p>
          ...
          {/* Recent Bookings */}
          <section>
            <SectionHeader title="Recent Bookings" to="/customer/bookings" linkLabel="View all" />
            {recentLoading ? (
              <div className="space-y-3">{[1, 2, 3].map((i) => <SkeletonCard key={i} />)}</div>
              ...
              <div className="space-y-3">
                {recentBookings.map((booking) => {
                  const serviceName = booking.serviceId?.name || booking.serviceName || "Service";
                  const date = booking.scheduledDate
                    ...
                    : "—";
                  return (
                    <div key={booking._id} className="bg-white border border-slate-200 rounded-xl px-4 py-3.5 flex items-center justify-between gap-4 hover:shadow-sm transition-shadow">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="text-xs font-semibold text-slate-700 truncate">{serviceName}</p>
                          <p className="text-xs text-slate-400 mt-0.5">{date}</p>
                        </div>
                      </div>
