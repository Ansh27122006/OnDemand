...
const AdminManageProducts = () => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/admin/products/${confirmProduct._id}`);
      setProducts((prev) => prev.filter((p) => p._id !== confirmProduct._id));
      setToast({
        message: `"${confirmProduct.name}" has been deleted.`,
        type: "success",
      });
    ...
    <div className="min-h-screen bg-slate-50">
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredProducts.length === 0 ? (
                  ...
                  filteredProducts.map((product) => {
                    const storeName =
                      product.vendorId?.storeName || product.storeName || "—";
                    ...
                    return (
                      <tr
                        ...
                        {/* Stock */}
                        <td
                          className={`px-5 py-4 whitespace-nowrap text-sm font-semibold ${stockStyle}`}>
                          {stock === 0
                            ? "Out of stock"
                            : typeof stock === "number" && stock <= 5
                            ? `${stock} (low)`
      ...
      {confirmProduct && (
        <ConfirmModal
          ...
          onCancel={() => !deleting && setConfirmProduct(null)}
        />
      )}
