...
const Toast = ({ message, type, onClose }) => {
  ...
  return (
    <div className={`fixed top-5 right-5 z-50 flex items-center gap-3 px-5 py-3.5 rounded-2xl shadow-xl text-sm font-medium border backdrop-blur-sm transition-all duration-300 ${
      type === "success"
      ...
      {message}
      <button onClick={onClose} className="ml-2 opacity-50 hover:opacity-100 transition-opacity text-base leading-none">✕</button>
    </div>
  );
};
  ...
  return (
    <div className="group bg-white rounded-3xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
      ...
      <div className="relative h-52 bg-gradient-to-br from-rose-50 to-orange-50 overflow-hidden">
        ...
        {/* Discount badges */}
        {discount > 0 && (
          <span className="absolute top-3 left-3 px-2.5 py-1 bg-red-500 text-white text-xs font-bold rounded-full shadow-sm">
            {discount}% OFF
          </span>
        )}
        {isStoreSale && (
  ...
  <div className="flex flex-col items-center justify-center py-24 px-4 text-center">
    <div className="relative mb-6">
      <div className="w-24 h-24 rounded-full bg-rose-50 flex items-center justify-center">
        <svg className="w-12 h-12 text-rose-300" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" />
        </svg>
      </div>
      <div className="absolute -top-1 -right-1 w-8 h-8 rounded-full bg-orange-100 flex items-center justify-center text-lg">✨</div>
    </div>
    <h3 className="text-2xl font-extrabold text-gray-800 mb-2">Your wishlist is empty</h3>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm mt-0.5">Products you've saved for later</p>
            </div>
          </div>
